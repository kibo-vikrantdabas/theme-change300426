define(['modules/jquery-mozu', 'underscore', 'modules/backbone-mozu', 'hyprlive', 'hyprlivecontext', 'modules/models-address'], function ($, _, Backbone, Hypr, HyprLiveContext, Address) {
    ...
    var title = Backbone.MozuModel.extend({
        validation: {
            titleName: {
                required: false,
    ...
    CreditCard = PaymentMethod.extend({
        ...
        validation: {
            ...
            nameOnCard: {
                fn: "present",
                required: true,  // this is cant be false we check this validation everywhere where we take cards to keeping it true again.
        ...
        presentCardType: function (value, attr) {
            if (!value) return Hypr.getLabel('cardNumberValidate');
        },
        checkCardNumber:function(value,attr){
            if (!value) return Hypr.getLabel('cardNumberMissing');
            var numbers = /^[0-9]+$/;
            if(!value.match(numbers)){
                if(value.indexOf('*********') > -1){
                       // valid masked card
                }else{
                    return Hypr.getLabel('cardNumberValidate');
                }
        ...
        expirationDateInPast: function (value, attr, computedState) {
                if (!value) return Hypr.getLabel('cardExpRequired');
