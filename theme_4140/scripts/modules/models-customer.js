define(['modules/backbone-mozu', 'underscore', 'modules/models-address', 'modules/models-orders', 'modules/models-paymentmethods', 'modules/models-product', 'modules/models-returns', 'hyprlive', 'modules/models-b2b-account', 'modules/api','modules/jquery-mozu', 'modules/country-data', 'hyprlivecontext'], function (Backbone, _, AddressModels, OrderModels, PaymentMethods, ProductModels, ReturnModels, Hypr, B2BAccountModels, api, $, CountryData, HyprLiveContext) {
    ...
    Customer = Backbone.MozuModel.extend({
        ...
        getContacts: function () {
            ...
            return this.apiGetContacts().then(function (cc) {
                var updatedContact = cc.data.items.filter(function(item){ return item.address.countryCode === Hypr.getThemeSetting('countrySpecificCode');});
                contactsCollection.reset(updatedContact);
                self.trigger('sync', cc.data);
                return self;
            });
        },
    ...
    EditableCustomer = Customer.extend({
        ...
        saveCard: function() {
            if (!this.validate('editingCard')) {
                var self = this,
                    ...
                    doSaveCard = function() {
                        return self.apiSavePaymentCard(editingCard).then(function(res) {
                            self.get('editingCard').set('cardId' , res.data.id);
                ...
                else if(editingCard.contactId && editingCard.contactId !== "new"){
                    saveContactOp = saveContactFirst();
                    if (saveContactOp) return saveContactOp.then(doSaveCard);
                } else {
                    return doSaveCard();
        ...
        updateCountryCities: function() {
            var currentCountryCitiesData = CountryData();
            if(currentCountryCitiesData) {
                this.get("editingContact").get('address').set('countryCode', currentCountryCitiesData.countryCode);
                this.get("editingContact").set('selectedAddressCity.cities', currentCountryCitiesData.cities);
            }
        },
