...
var CheckoutPage = Backbone.MozuModel.extend({
            ...
            ensureEmailIsSet: function () {
              ...
              //  var billingEmail = this.get('billingInfo.billingContact.email'),
                  var  customerEmail = require.mozuData('user').email,
            ...
            submit: function () {
                var checkout = this,
                    ...
                    // Validate National Address Short Code (address4) if required
                var isShortCodeRequired = HyprLiveContext.locals.themeSettings.nationalAddressShortCodeRequired;
                if (isShortCodeRequired && requiresFulfillmentInfo) {
                    var fulfillmentContact = this.get('fulfillmentInfo') ? this.get('fulfillmentInfo').get('fulfillmentContact') : null;
                    var address4Value = fulfillmentContact && fulfillmentContact.get('address') ? fulfillmentContact.get('address').get('address4') : null;
                    
                    if (!address4Value) {
                        this.trigger('error', {
                            message: Hypr.getLabel('nationalAddressShortCodeMissing')
                        });
                    ...
                    if (!shortCodePattern.test(address4Value.toUpperCase())) {
                        this.trigger('error', {
                            message: Hypr.getLabel('nationalAddressShortCodeInvalidFormat')
                        });
                        this.isSubmitting = false;
                        return false;
                    }
                ...
                if (activePayments !== null && activePayments.length > 0) {
                     ...
                     if (creditCard && creditCard.billingInfo && creditCard.billingInfo.card) {
                         saveCreditCard = true;//creditCard.billingInfo.card.isCardInfoSaved;
