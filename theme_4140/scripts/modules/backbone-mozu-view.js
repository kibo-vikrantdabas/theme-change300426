    ...
    var MozuView = Backbone.MozuView = Backbone.View.extend(
        ...
        handleLoadingChange: function (isLoading) {
            if(require.mozuData('pagecontext').cmsContext)
                if(require.mozuData('pagecontext').cmsContext.template.path == 'cart')  {
                   if(this.model.get('isWishlistAction') || this.model.get('isRemoveFromCartAction') || this.model.get('isFulfillmentMethodAction')) isLoading = false;
                }
