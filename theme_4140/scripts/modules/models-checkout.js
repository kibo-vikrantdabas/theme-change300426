define([
    ...
    'modules/checkout/payment/SplitPayment',
    'modules/card-token',
    'modules/garbage-dump'
],
    function ($, _, Hypr, Backbone, api, CustomerModels, AddressModels, PaymentMethods, HyprLiveContext,CheckoutGTM, CheckoutEvents, SplitPaymentHandler, CardTokenHandler, GarbageHandler) {
        var helperAction = 0;
        var CheckoutStep = Backbone.MozuModel.extend({
            helpers: ['stepStatus', 'requiresFulfillmentInfo','isAwsCheckout','isNonMozuCheckout', 'requiresDigitalFulfillmentContact','isShippingEditHidden', 'isCNC'],  //
            ...
            isCNC : function() {
                var flag = _.chain(this.getOrder().get('items')).first().pick('fulfillmentMethod').value().fulfillmentMethod;
                if(flag) {
                    ...
                    if(!_.isNull(selectedStoreData)) {
                        ...
                        $(".mz-pickupLocationName").text(selectedStoreData.name);
                        $(".mz-pickupLocationAddress").text(selectedStoreData.address);
                    }
                }
                return flag.toLowerCase() == "pickup" ? true : false ;
            },
        }),
        ...
        FulfillmentContact = CheckoutStep.extend({
            ...
            contacts: function () {
                ...
                contacts = contacts.filter(function(item){ return item.address.countryCode === Hypr.getThemeSetting('countrySpecificCode');});
        ...
        FulfillmentInfo = CheckoutStep.extend({
            ...
            applyShipping: function(resetMessage) {
                ...
                // Check if nationalAddressShortCodeRequired is enabled and address4 is missing
                var nationalAddressShortCodeRequired = HyprLiveContext.locals.themeSettings.nationalAddressShortCodeRequired;
                if (nationalAddressShortCodeRequired) {
                    var fulfillmentContact = this.get('fulfillmentContact');
                    var address = fulfillmentContact ? fulfillmentContact.get('address') : null;
                    var address4 = address ? address.get('address4') : null;
                    if (!address4 || !address4.trim()) {
                        // Don't make API call if address4 is required but missing
                        // This prevents updating the order with incomplete address data
                        return false;
                    }
                }
                this.isLoading(true);
        ...
        BillingInfo = CheckoutStep.extend({
            ...
            helpers: ['savedPaymentMethods', 'availableStoreCredits', 'applyingCredit', 'maxCreditAmountToApply',
              'activeStoreCredits', 'activeGiftCards', 'nonStoreCreditOrGiftCardTotal', 'activePayments', 'hasSavedCardPayment', 'availableDigitalCredits', 'availableGiftCards', 'digitalCreditPaymentTotal', 'giftCardPaymentTotal', 'isAnonymousShopper', 'visaCheckoutFlowComplete', 'isBenefitPayApply', 'isOmanNetApply', 'isQpayApply', 'isKNETPaymentType', 'isApplePayPaymentType', 'isExternalCheckoutFlowComplete', 'checkoutFlow', 'customerBlueNumber', 'shouldCODDisplay', 'voidExistingPayment', 'setEPGCardDetails'],

            setEPGCardDetails: function() {
                if(sessionStorage.getItem('TransId')) {
                     this.get('card').set('cvv', '***');
                     //this.set('isCreditCardPayment', true);
                     if (this.get('isKNETPaymentType') || this.get('isBenefitPayApply') || this.get('isQpayApply') || this.get('isApplePayPaymentType') || this.get('isOmanNetApply')) {
                        this.set('isCreditCardPayment', false); // ✅ prevent showing in credit card UI
                    } else {
                        this.set('isCreditCardPayment', true); // ✅ regular credit card flow
                    }
                }
            },
                
            
            shouldCODDisplay : function() {
                var flag = _.chain(this.getOrder().get('items')).first().pick('fulfillmentMethod').value().fulfillmentMethod;
                return (flag.toLowerCase() == "pickup" ? false : true) && (Hypr.getThemeSetting('isCODDisable') ? false : true);
            },
            voidExistingPayment: function(){
                if(helperAction < 2 ){
                    var isDefaultPYamethodSet = window.order.get('billingInfo').get('card').get('isDefaultPayMethod');
                    
                    if(isDefaultPYamethodSet){
                        if(!_.isEmpty(window.order.apiModel.getActivePayments())){
                           if(_.isNull(sessionStorage.getItem('splitPayments')) && _.isNull(sessionStorage.getItem('TransId'))) window.order.apiVoidPayment(window.order.apiModel.getActivePayments()[0].id);
                        }
            ...
            customerBlueNumber:function() {
                var self = this;
                _.each(this.getOrder().get('customer').get("attributes").models, function(value){
                    if( _.isEqual(value.get("fullyQualifiedName").toLowerCase(), "tenant~customer-registered-mobile") ) {
                        if(!_.isEmpty(value.get("values"))) self.set("customerBlueNumber", _.first(value.get("values")));
                    }
                });
            },
            ...
            isBenefitPayApply: function () {
                return sessionStorage.getItem('isBenefitPayApply') === 'true';
              },
              isQpayApply: function () {
                return sessionStorage.getItem('isQpayApply') === 'true';
              },
            isOmanNetApply: function () {
                return sessionStorage.getItem('isOmanNetApply') === 'true';
            },
             isKNETPaymentType: function () {
                return sessionStorage.getItem('isKNETPaymentType') === 'true';
              },
               isApplePayPaymentType: function () {
                return sessionStorage.getItem('isApplePayPaymentType') === 'true';
              },
            ...
            avoidCheckPaymentMethod: function() {
                var self = this;
                var order = this.getOrder();
                var currentPayment = order.apiModel.getCurrentPayment();
                if(currentPayment.paymentType == "Check") {
                    return order.apiVoidPayment(currentPayment.id).then(function() {
                        order.update();
                    });
                }
           ...
           retrieveGiftCard: function(number, securityCode) {
            ...
            setPurchaseOrderInfo: function() {
                var me = this,
                    ...
                    function updateFulfillmentMethod(data) {
                        if (!data || !data.attributes || !data.attributes.items || !data.attributes.items.length) return;
                        var firstMethod = data.attributes.items[0].fulfillmentMethod;
                        data.attributes.items.slice(1).forEach(function(item) {
                            item.fulfillmentMethod = firstMethod;
                        });
                    }
            ...
            setPurchaseOrderBillingInfo: function() {
                var me = this,
                    ...
                    purchaseOrderEnabled = this.isPurchaseOrderEnabled(),
                    currentPurchaseOrder = me.get('purchaseOrder'),
                    contacts = order ? order.get('customer').get('contacts').filter(function(item){ return item.get("address").get("countryCode") === Hypr.getThemeSetting('countrySpecificCode');}) : null;
                if(purchaseOrderEnabled) {
            ...
            initialize: function () {
                ...
                _.defer(function () {
                    ...
                    me.set('isCreditCardPayment',false); 
                    me.set('isSavedCardPresent',false);
                    if (me.savedPaymentMethods() && me.savedPaymentMethods().length > 0) {
                        me.set('saveCardFlag',true);
                        me.set('IsSavedCard',true);
                        me.set('addNewFlag',false);
                        me.set('isSavedCardPresent',true);
                    }
            ...
            setSaveCardFlag:function(me){
                me.set('saveCardFlag',false);
            },
            selectPaymentType: function(me, newPaymentType) {
                ...
                var isCardLength = 0;
                if(window.order) isCardLength = window.order.get('customer').get('cards').length;

                if(newPaymentType === 'PurchaseOrder') me.setPurchaseOrderBillingInfo();

                 if(newPaymentType === 'CreditCard'){
                    me.set('isCreditCardPayment',true);
                    me.set('isSameBillingShippingAddress',true);     
                    me.set('showNewAddress',true); 
                    me.set('showSavedCardForm',true); 
                    // me.set('usingSavedCard', true);
                    if(me.get('card').get('paymentOrCardType') === "BLUEPREPAIDCARD"){
                        me.set('isCreditCardPayment',false);
                        me.set('showNewAddress',false);
                    }
                      if (me.get('isBenefitPayApply') || me.get('isQpayApply') || me.get('isOmanNetApply') || me.get('isKNETPaymentType') || me.get('isApplePayPaymentType') ){
                        console.log("✅ BenefitPay or OmanNet or Qpay selected, skipping credit card UI");
                        me.set('isCreditCardPayment', false);
                        me.set('showNewAddress', false);
                     }
                    // condition to handle multipayment ui
                    if (!(this.get('isBenefitPayApply') || this.get('isQpayApply')|| this.get('isOmanNetApply') || me.get('isKNETPaymentType') || me.get('isApplePayPaymentType'))) {
                    if(isCardLength > 0){
                       
                        me.set('usingSavedCard', true);
                        me.get('card').set('hideSavedCard',false);
                    }else{
                        me.set('usingSavedCard', false);
                        me.get('card').set('hideSavedCard',true); // this is to view saved card link only when user has saved card
                    }
                    }
                 ...
                 if(newPaymentType === 'Check'){ 
                    ...
                    if(user.isAuthenticated) {
                        ...
                        if(me.parent) {
                            var shippingContact = me.parent.get('fulfillmentInfo').get('fulfillmentContact').toJSON();
                            var billingContact = me.get('billingContact');

                            billingContact.get('address').set('address1', shippingContact.address.address1);
                            billingContact.get('address').set('address2', shippingContact.address.address2);
                            billingContact.get('address').set('address3', shippingContact.address.address3);
                            billingContact.get('address').set('addressType', shippingContact.address.addressType);
                            billingContact.get('address').set('candidateValidatedAddresses', shippingContact.address.candidateValidatedAddresses);
                            billingContact.get('address').set('cityOrTown', shippingContact.address.cityOrTown);
                            billingContact.get('address').set('countryCode', shippingContact.address.countryCode);
                            billingContact.get('address').set('defaultAddress', shippingContact.address.defaultAddress);
                            billingContact.get('address').set('isValidated', shippingContact.address.isValidated);
                            billingContact.get('address').set('postalOrZipCode', shippingContact.address.postalOrZipCode);
                            billingContact.get('address').set('stateOrProvince', shippingContact.address.stateOrProvince);
                            billingContact.get('phoneNumbers').set('home', me.get('customerBlueNumber'));
                            billingContact.set('firstName', shippingContact.firstName);
                            billingContact.set('lastNameOrSurname', shippingContact.lastNameOrSurname);
                        }
                    }
                 }
            },
            ...
            submit: function () {   
                var card1 = this.get('card');
                var order = this.getOrder();
                if(order.get('isCOD')) {
                    this.set('paymentType','Check');
                    this.unset('card');
                 }
                // just can't sync these emails right
                order.ensureEmailIsSet();
                var cardData; 
                if( !order.get('isCOD') )  cardData  = this.get('card');
                ...
                if(cardData){
                    ...
                    if(cardData.get('cardNumber') === undefined) cardData.set('isSavedCard',false);

                    if(cardData.get('paymentOrCardType') === ''){
                        cardData.set('cardNumber', '');
                        cardData.set('cardNumberPart', '');
                    }

                    if(cardData.get('expireYear') !== undefined){
                        if(cardData.get('expireYear').toString().length == 2){
                            var newExpireYear = 20 + cardData.get('expireYear').toString();
                            cardData.set('expireYear', newExpireYear);
                    }

                    if(cardData.get('expireMonth') !== undefined){
                       if(cardData.get('expireMonth')<10){
                            var newExpireMonth = 0 + cardData.get('expireMonth').toString();
                            cardData.set('expireMonth', newExpireMonth);
                        }
                        this.set('cardData', cardData.toJSON());
                    }
                  }
                }
                ...
                if (currentPayment) this.get('card').set('isVisaCheckout', currentPayment.paymentWorkflow.toLowerCase() === 'visacheckout');

                var val;


                if (this.get('isKNETPaymentType') || this.get('isApplePayPaymentType') || this.get('isBenefitPayApply') || this.get('isQpayApply') || this.get('isOmanNetApply')) {

                    if( this.get('isApplePayPaymentType')){
                       // sessionStorage.setItem(this.get('isKNETPaymentType') ? 'isKNETPaymentType' : 'isApplePayPaymentType', true);
                                            //  sessionStorage.setItem(this.get('isKNETPaymentType') ? 'isKNETPaymentType' : 'isApplePayPaymentType', true);
  const card = this.get('card');
   this.set('usingSavedCard', false);
  this.get('card').set('hideSavedCard', true);
  this.get('card').set('issavedCard', false);
                        this.get('card').set('cardNumber', "4111111111111111");
                        this.get('card').set('cardNumberPartOrMask', "4111111111111111");
                        this.get('card').set('paymentOrCardType', this.get('isKNETPaymentType') ?'KNETCARD' : "APPLEPAY");
                        this.get('card').set('cvv', '123');
                        this.get('card').set('expireYear', (new Date().getFullYear()+10));
                        this.get('card').set('expireMonth', 12);
                        this.get('card').set('nameOnCard', this.get('isKNETPaymentType') ? 'KNETTESTUSER' : 'APPLEPAYUSER');
                        this.set('cardData', this.get('card').toJSON());
                         this.set('paymentType', 'CreditCard');
                    }
                    if (this.isKNETPaymentType()) {
                   //     sessionStorage.setItem(this.get('isKNETPaymentType') ? 'isKNETPaymentType' : 'isApplePayPaymentType', true);
  const card = this.get('card');
   this.set('usingSavedCard', false);
  this.get('card').set('hideSavedCard', true);
  this.get('card').set('issavedCard', false);
//  this.set('savedPaymentMethodId', null);
  card.set('cardNumber', '4111111111111111');
  card.set('cardNumberPartOrMask', '4111111111111111');
  card.set('paymentOrCardType', 'KNETCARD');
  card.set('cvv', '123'); // not '***'
  card.set('expireYear', (new Date().getFullYear() + 10));
  card.set('expireMonth', 12);
  card.set('nameOnCard', 'KNETTESTUSER');
  this.set('cardData', card.toJSON());
  this.set('paymentType', 'CreditCard');
}

                    

                     console.log("Card Info Post Login", this.get('card').toJSON());
                    // console.log("BenefitPay Flag Post Login", this.get('isBenefitPayApply'));

                     if (this.get('isBenefitPayApply')) {
                         console.log("Came inside benefit" + this.get('isBenefitPayApply'));
                         //sessionStorage.setItem('isBenefitPayApply', true);
                        this.get('card').set('cardNumber', "4600410123456789");
                        this.get('card').set('cardNumberPartOrMask', "4600410123456789");
                        this.get('card').set('paymentOrCardType',"BENEFITCARD");
                        this.get('card').set('cvv', '1234');
                        this.get('card').set('expireYear', (new Date().getFullYear()+10));
                        this.get('card').set('expireMonth', 12);
                        this.get('card').set('nameOnCard', "BENEFIT");
                        this.set('cardData', this.get('card').toJSON());
                        this.set('paymentType', 'CreditCard');
                    }

                    if (this.get('isQpayApply')) {
                        console.log("Came inside Qpay: " + this.get('isQpayApply'));    
                          // sessionStorage.setItem('isQpayApply', true);
                        this.get('card').set('cardNumber', "5105105105105100");
                        this.get('card').set('cardNumberPartOrMask', "5105105105105100");
                        this.get('card').set('paymentOrCardType',"QPAY");
                        this.get('card').set('cvv', '123');
                        this.get('card').set('expireYear', (new Date().getFullYear()+10));
                        this.get('card').set('expireMonth', 12);
                        this.get('card').set('nameOnCard', "QPAY");
                        this.set('cardData', this.get('card').toJSON());
                        this.set('paymentType', 'CreditCard');
                    }

                    //console.log("is this OmanNet: " + this.get('isOmanNetApply'));
                    if (this.get('isOmanNetApply')) {
                        console.log("Came inside OmanNet: " + this.get('isOmanNetApply'));
                       // sessionStorage.setItem('isOmanNetApply', true);
                        this.get('card').set('cardNumber', "4228230000000001");
                        this.get('card').set('cardNumberPartOrMask', "4228230000000001");
                        this.get('card').set('paymentOrCardType',"OMANNET");
                        this.get('card').set('cvv', '123');
                        this.get('card').set('expireYear', "2039");
                        this.get('card').set('expireMonth', "01");
                        this.get('card').set('nameOnCard', "OMANNET");
                        this.set('cardData', this.get('card').toJSON());
                        this.set('paymentType', 'CreditCard');
                        this.set('cardData', this.get('card').toJSON());
                    }
                      ...
                      // After the saved card logic runs
                if (this.get('isBenefitPayApply') || this.get('isOmanNetApply') || this.get('isQpayApply') || this.get('isKNETPaymentType') || this.get('isApplePayPaymentType')) {
                        this.set('usingSavedCard', false);
                        this.get('card').set('hideSavedCard', true);
                        this.get('card').set('isSavedCard', false); // ensure it’s treated as a new card
                    }
                console.log("it came till here")
                if(!order.get('isCOD'))  val = this.validate();
                ...
                if(this.get('paymentType').toLowerCase() === "purchaseorder") this.get('purchaseOrder').inflateCustomFields();
            ...
            applyPayment: function () {
                var self = this, order = this.getOrder();
                this.set('disablePaymentOption', true);
                if (this.get("paymentWorkflow") == "PayWithAmazon") this.unset("paymentWorkflow");

                this.syncApiModel();
                
                this.checkAndExecuteSplitPayment(order);

                if (this.nonStoreCreditOrGiftCardTotal() > 0) {
                    return order.apiAddPayment().then(function(res) {
                        var payment = order.apiModel.getCurrentPayment();
                        ...
                        if (!creditCardPayment && self.get('card')) {
                            ...
                            window.order.set('paymentServiceCardId', creditCardPayment.billingInfo.card.paymentServiceCardId);
                        }
                        ...
                        if (payment) {
                            ...
                            switch (payment.paymentType) {
                                case 'CreditCard':
                                    self.set('isCheckPayment',false); 
                                    modelCard = self.get('card');
                                    modelCvv = modelCard.get('cvv');

                                    if (modelCvv && modelCvv.indexOf('*') === -1 ) modelCard.set('cvv', '***');
                                    if(self.get('isKNETPaymentType')) self.set('hasPaymentDone', true);
                                    if(self.get('isBenefitPayApply')) self.set('hasPaymentDone', true);
                                    if(self.get('isQpayApply')) self.set('hasPaymentDone', true);
                                    if(self.get('isOmanNetApply')) self.set('hasPaymentDone', true);
                                    if(self.get('isApplePayPaymentType')) self.set('hasPaymentDone', true);
                                    self.markComplete();

                                    break;
                                case 'Check':
                                    self.set('isCreditCardPayment',false); 
                                    self.set('isCheckPayment',true); 
                                    self.set('hasPaymentDone', true);
                                    self.stepStatus("incomplete");
                                    break;
                                default:
                                    self.markComplete();
                    ...
                    // ✅ Also enforce for 0-value payments (e.g., 100% covered by gift card)
        if (self.get('isBenefitPayApply') || self.get('isOmanNetApply') || this.get('isQpayApply')|| self.get('isKNETPaymentType')  || self.get('isApplePayPaymentType')) {
            self.set('isCreditCardPayment', false);
            self.set('usingSavedCard', false);
            self.set('showNewAddress', false);
        }
                    this.markComplete();
                }
            },
            checkAndExecuteSplitPayment: function(order) {
                if(!_.isEmpty(window.order.get("bluePayments"))) { 
                    var totalPay = 0,
                    prepaidCardPay = order.get('bluePrepaidCard'),
                    blueWalletPay = order.get('blueWallet');
                       
                    if(!_.isUndefined(prepaidCardPay)) totalPay += prepaidCardPay;
                        
                    if(!_.isUndefined(blueWalletPay)) totalPay += blueWalletPay;

                    if(totalPay < order.get("total")) {
                        SplitPaymentHandler.init.apply(this, [order.get("billingInfo"), order.get("id"), order.get("total")]);
                    }
                    else {
                        if(this.has('bluePrepaidCardDeductAmount')) {
                            this.unset('bluePrepaidCardDeductAmount');
                            order.unset('bluePrepaidCard');
                        }
                        
                        if(this.has('blueWalletDeductAmount')) {
                            this.unset('blueWalletDeductAmount');
                            order.unset('blueWallet');
                        }
                    }
                }
            },
            markComplete: function () {
                if(!window.order.get("isCOD")) this.stepStatus('complete'); 

                this.set('hasPaymentDone', true);
                this.set('disablePaymentOption', false);
                
                if(this.get('isKNETPaymentType')) this.parent.initializeLoader('deactive', 'active');

                if(this.get('isBenefitPayApply')) this.parent.initializeLoader('deactive', 'active');

                if(this.get('isQpayApply')) this.parent.initializeLoader('deactive', 'active');

                if(this.get('isOmanNetApply')) this.parent.initializeLoader('deactive', 'active');

                if(this.get('isApplePayPaymentType')) this.parent.initializeLoader('deactive', 'active');
                ...
                if(!window.order.get('isPaymentServiceIdExist') && window.order.get('splitPayments')){
                    SplitPaymentHandler.addPaymentServiceId.call(this);
                    SplitPaymentHandler.addCardMultiPayment(window.order.get('id'), window.order.get('splitPayments'));
                }
            },
            toJSON: function(options) {
                var j = CheckoutStep.prototype.toJSON.apply(this, arguments);

                if (this.nonStoreCreditOrGiftCardTotal() === 0 && j.billingContact) delete j.billingContact.address;
        ...
        var CheckoutPage = Backbone.MozuModel.extend({
            ...
            initialize: function (data) {
                ...
                _.defer(function() {
                    ...
                    self.get('billingInfo.billingContact').on('change:email', function(model, newVal) { self.set('email', newVal); });

                    self.applyAttributes();

                });

                if (user.isAuthenticated) this.set('customer', { id: user.accountId });
                ...
                _.bindAll(this, 'update', 'onCheckoutHandle', 'onCheckoutError', 'addNewCustomer', 'saveCustomerCard', 'apiCheckout',
            ...
            addCoupon: function () {
                ...
                var existingCodes = me.get('couponCodes') || [];

                me.isLoading(true);

            // Step 1: Remove existing coupon (if any)
           var removePromise = existingCodes.length > 0
                 ? me.apiRemoveCoupon(existingCodes[0])
                 : Promise.resolve();

            return removePromise.then(function () {
            // Step 2: Apply new coupon
          return me.apiAddCoupon(code).then(function () {
            if (me.get('billingInfo')) {
                console.log("[BillingInfo] Exists, triggering sync.");
                me.get('billingInfo').trigger('sync');
            }// optional chaining in case billingInfo isn't defined
            me.set('couponCode', '');
            console.log("[Coupon] Coupon code input cleared.");
              ...
              //  return this.apiAddCoupon(this.get('couponCode')).then(function () {
                    ...
                    if (me.get('suggestedDiscounts').length && autoAddSuggestedDiscount){
                        console.log("[Discount] Auto-add suggested discount detected. Redirecting to cart.");
                    ...
                    console.log("[Discounts] Total discounts collected:", allDiscounts.length);

                    if (!allDiscounts || !_.find(allDiscounts, matchesCode))
                    {
                        console.log("[Error] No matching discount found for coupon:", code)
                        me.trigger('error', {
                    ...
                    console.log("[Success] Coupon applied successfully:", code);
                    if (me.get('total') === 0) {
                        console.log("[Order] Total is zero. Triggering complete.");
                        me.trigger('complete');
                    }
                    // only do this when there isn't a payment on the order...
                    me.get('billingInfo').updatePurchaseOrderAmount();
                    console.log("[BillingInfo] Purchase order amount updated.");
                }
                    if(window.globalEventBus){
                        var eventData = {
                            'custom_event': 'checkout page',
                            'event_params':{
                                'event_act': 'promocode',
                                'event_lbl':  code
                            }
                          };
                          console.log("[Event] Emitting dataLayerEvent:", eventData);
                        window.globalEventBus.emit('dataLayerEvent', eventData);
                    }
                      
                    me.isLoading(false);
                    console.log("[UI] Loading state set to false.");

                    if ($(window).width() < 1001) {
                        console.log("[UI] Mobile view adjustments applied.");
                ...
                console.error("[Coupon] Failed to apply coupon. Error:", error);
                me.isLoading(false);
            });
            },
            // onCheckoutHandle: function (res) {
            //     if(res.data) {
            //         if(res.data.status === 'Errored' || res.data.paymentStatus === 'Errored') {
            //             this.isLoading(true);
            //             this.initializeLoader('deactive','active');
            //             var original = location.href;
            //             var replaceWith = window.location.href.substring(window.location.href.lastIndexOf('/') + 1).split("?")[0];
            //             var stringToFind = replaceWith;
            //             var indexOf = original.indexOf(stringToFind);
            //             original = original.replace(original.substring(indexOf, indexOf + stringToFind.length), "");
            ...
            //             this.onCheckoutError({message: Hypr.getLabel('paymentAuthCreditCardError'), items: []});
            //              return;
            //         } else {
            //             this.onCheckoutSuccess(res)
            //         }
            //     }
            // },
            onCheckoutHandle: function (res) {
               console.log("final response" + JSON.stringify(res));
                if (res.data) {
                    var paymentStatus = res.data.paymentStatus;
            
                    if (this.isEPGTimeout(res.data)) { 
                        console.log("✅ Payment marked as timeout, treating order as successfully placed");
                    
                        // ✅ Trigger checkout success flow
                        this.onCheckoutSuccess(res);  
                    
                        this.isLoading(false);
                        this.initializeLoader('active', 'deactive');
                        this.trigger('complete');
                    
                        setTimeout(function () {
                            if (res.data && res.data.orderNumber) {
                                var confirmationUrl = (HyprLiveContext.locals.siteContext.siteSubdirectory || '') + '/checkout/' + res.data.orderNumber + '/confirmation';
                                console.log("✅ Redirecting manually to confirmation page:", confirmationUrl);
                    ...
                    if (res.data.status === 'Errored' || paymentStatus === 'Errored') {
                        this.isLoading(true);
                        this.initializeLoader('deactive', 'active');
                        ...
                        this.onCheckoutError({
                            message: Hypr.getLabel('paymentAuthCreditCardError'),
                            items: []
                        });
                        return;
                    }
            ...
            isEPGTimeout: function (orderData) {
                console.log("🔍 Checking for EPG timeout...");
                try {
                    if (!orderData || !orderData.payments) return false;
            
                    return orderData.payments.some(function (payment) {
                        return payment.interactions && payment.interactions.some(function (interaction) {
                            var respCode = interaction.gatewayResponseCode;
                            var respText = interaction.gatewayResponseText ? interaction.gatewayResponseText.toLowerCase() : '';
                            return respCode === '007';
                        });
                    });
                } catch (e) {
                    console.warn("⚠️ Error in isEPGTimeout check:", e);
                    return false;
                }
            },
             onCheckoutSuccess: function (res) {
                window.CartPopoutInstance.model.clear();
                        var userInfo = require.mozuData('user');
                        GarbageHandler.session();
                        var awaitFlag = true;
                        if(userInfo.isAuthenticated) {
                           if(window.order.get('billingInfo').get("paymentType").toLowerCase() === "creditcard") {
                               var cardInfo = window.order.get('billingInfo').get("card");
                               if(cardInfo) {
                                   cardInfo = cardInfo.toJSON();
                                   
                                if(cardInfo.cardType.toLowerCase() !== "knetcard" && cardInfo.cardType.toLowerCase() !== "applepay" && cardInfo.cardType.toLowerCase() !== "benefitcard" && cardInfo.cardType.toLowerCase() !== "omannet" && cardInfo.cardType.toLowerCase() !== "qpay") {
                                    
                                    var  selectASavedCardCookie = $.cookie('checkboxState');
                                    awaitFlag = false;
                                    
                                   
                                    this.saveCustomerCard().then(function(data) {
                                        awaitFlag = true;
                                        ...
                                        this.initializeLoader('deactive','active');
                                        this.trigger('complete');
                                        $('.place-order-btn').addClass('loader-btn');
                                        $('.place-order-btn span').hide();
                                        setTimeout(function(){
          ...
          onCheckoutError: function (error) {
              ...
              var errorMessageNode = $('#checkout-form .mz-messagebar .mz-errors .mz-message-item');
              var errorMessageServer = "";
              if (error && error.items && error.items.length > 0 && error.items[0].message) {
                  errorMessageServer = error.items[0].message.toLowerCase();
              }
              var errorNameServer = "";
              if (error && error.items && error.items.length > 0 && error.items[0].name) {
                  errorNameServer = error.items[0].name.toLowerCase();
              }
              console.log("Error messageServer: " + errorMessageServer);
              console.log("Error nameServer: " + errorNameServer);

              if (errorMessageServer.includes("server is not reachable") || errorNameServer.includes("session error")) {
                  console.log("it came inside the not reachable");
                  order.messages.reset([{
                      message: Hypr.getLabel('paymentServerFailure')
                  }]);
                  order.isSubmitting = false;
                  $('.place-order-btn').removeClass('loader-btn');
                  $('.place-order-btn span').show();
                  return;
              }

                if(errorMessage.includes('contact support') || errorMessage.includes('user not authorized') || errorMessage.includes('order can not be edited') || errorMessageNode.text()){
                    
                    $('.mz-redirection').removeClass('active');
                    setTimeout(function(){
                       window.location = (HyprLiveContext.locals.siteContext.siteSubdirectory || '') + "/cart";
                    },2500);
                }
                ...
                if(error.errorCode == "FORBIDDEN" && window.order.get("billingInfo").get("isCheckPayment")) {
                    window.location.reload();
                }

                if('errorCode' in error) {
                     GarbageHandler.session();
                     this.reloadAndSetErrorFlag();
                }

                
                if(error.message){
                    var errorMessageMargin = error.message.length;
                    if (window.matchMedia('(max-width: 500px)').matches){
                        if(errorMessageMargin <= 30){
                            $('.mz-checkoutform-shippingaddress').css('margin-top','69px');
                        }else if(errorMessageMargin > 30 ){
                            $('.mz-checkoutform-shippingaddress').css('margin-top','103px');
                        }
                    }else{
                        $('.mz-checkoutform-shippingaddress').css('margin-top','0px');
                    }
                }
            ...
            reloadAndSetErrorFlag:function() {
                sessionStorage.setItem('isAuthError', true);
                this.get('billingInfo').set('isAuthError', false);
                location.reload();
            },
            reThrowAuthError:function() {
                this.initializeLoader('deactive','active');
            ...
            initializeLoader:function(addClass, removeClass){
                if($('.mz-redirection').hasClass(removeClass)){
                    $('.mz-redirection').addClass(addClass).removeClass(removeClass);
                }
            },
            ...
            addCustomerContact: function (infoName, contactName, contactTypes) {                
                var customer = this.get('customer'),
                    ...
                    process = [function () {
                        ...
                        // Validate if orderContact exists
                        if (!orderContact) {
                            
                            return;
                        }
                        // Validate the necessary fields in orderContact
                        if (!orderContact.name || !orderContact.address) {
                            
                        }
                        // Log the current orderContact.id value
                        
                        // Check if a valid orderContact.id exists and try to update the contact
                        ...
                        // If orderContact.id is invalid or indicates a new contact
                        if (orderContact.id === -1 || orderContact.id === 1 || orderContact.id === 'new') {
                            
                            delete orderContact.id;
                        }
                        // Log the decision to add a new contact
                        
                        // Try to add the contact
                        return customer.apiModel.addContact(orderContact)
                            .then(function (contactResult) {
                                if (!contactResult.data || !contactResult.data.id) {
                    ...
                    process.unshift(function() {
                        return customer.apiModel.getContacts().then(function(contacts) {
                            _.each(contactTypes, function(newType) {
                                ...
                                newType.isPrimary = orderContact.isMarkDefaultAddressSelected;
                            });
                        });
                    });
                } else {
                ...
                if(!orderContact) {
                    orderContact = {
                        "email": this.get('email') || this.get('emailAddress') || customer.get('emailAddress') || require.mozuData('user').email || $.cookie('guestUserEmail'),
                        "orderId": contactInfoContactName.get("orderId"),
                        "address": {
                            "candidateValidatedAddresses": null,
                            "countryCode": Hypr.getThemeSetting('countrySpecificCode'),
                            "addressType": "Residential",
                            "defaultAddress": false,
                            "address1": contactInfoContactName.get("address").get("address1"),
                            "address2": contactInfoContactName.get("address").get("address2"),
                            "address3": contactInfoContactName.get("address").get("address3"),
                            "postalOrZipCode": "00000",
                            "cityOrTown": contactInfoContactName.get("address").get("cityOrTown"),
                            "stateOrProvince": "n/a",
                        },
                        "phoneNumbers": {
                            "home": window.order.get("billingInfo").get("billingContact").get("phoneNumbers").get("home"),
                            "dialingCode": window.order.get("billingInfo").get("billingContact").get("phoneNumbers").get("dialingCode")
                        }
                    }
                    if(require.mozuData('user').isAuthenticated){
                        orderContact.userId = require.mozuData('user').userId;
                    }
                }
                // handle email
                
                if (!orderContact.email) orderContact.email = this.get('email') || this.get('emailAddress') || customer.get('emailAddress') || require.mozuData('user').email || $.cookie('guestUserEmail');
                ...
                if (!orderContact.id || orderContact.id === -1 || orderContact.id === 1 || orderContact.id === 'new') {
                    ...
                    var customerContact = customerContacts.get(orderContact.id) ? customerContacts.get(orderContact.id).toJSON(): customerContacts.get(orderContact.id);
                    if (customerContact && this.isContactModified(orderContact, customerContact)) {
            ...
            saveCustomerCard: function () {
                var order = this,
                    ...
                    doSaveCard = function() {
                        try {
                            ...
                            var  selectASavedCardCookie = $.cookie('checkboxState');
                            if(selectASavedCardCookie != 'checked'){
                ...
                if (!billingContact.id || billingContact.id === -1 || billingContact.id === 1 || billingContact.id === 'new') {
                    ...
                    return this.addCustomerContact("billingInfo", "billingContact", billingContact.types)
            ...
            setFulfillmentContactEmail: function () {
                ...
                if (!fulfillmentEmail) this.set('fulfillmentInfo.fulfillmentContact.email', orderEmail);

            },
            ensureEmailIsSet: function () {
                var  customerEmail = this.get('emailAddress') || require.mozuData('user').email || $.cookie('guestUserEmail');
            ...
            checkInventory:function(vpc,locationCode,redirectCart){
                var redirect=false;
                api
                .request(
                  "POST",
            ...
            submit: function () {
                var order = this,
                    ...
                    isCreditCardPayment =billingInfo.get('isCreditCardPayment'),
                    requiresBillingInfo = nonStoreCreditOrGiftCardTotal > 0,
                    payment=this.get('payments'),
                    paymentStatus=this.get('paymentStatus'),
                    isPaidByBluePrepaidCard,
                    isPaidByBlueWallet,
                    process = [function() {
                        return order.update({
                    ...
                    //  if (billingInfo.get('isBenefitPayApply') || billingInfo.get('isOmanNetApply')) {
                    //     billingInfo.set('isCreditCardPayment', false);
                    //     billingInfo.set('usingSavedCard', false);
                    //     billingInfo.set('showNewAddress', false);
                    //    } 

                    
                    isPaidByBluePrepaidCard = order.get("bluePrepaidCard") ? true : false;
                    isPaidByBlueWallet = order.get('blueWallet') ? true : false;

                    if(isPaidByBlueWallet) {
                        isSameBillingShippingAddress = false;
                        this.get('billingInfo').set('isSameBillingShippingAddress', isSameBillingShippingAddress);
                        var blueCustomerMobileNumber = billingInfo.get('customerBlueMobileNumber');
                        this.get('billingInfo').get('billingContact').get('phoneNumbers').set('home', blueCustomerMobileNumber);
                        billingInfo.get('billingContact').get('phoneNumbers').set('home', blueCustomerMobileNumber);
                        billingContact.get('phoneNumbers').set('home', blueCustomerMobileNumber);
                    }
                       
                    if(_.isEmpty(payment)){
                        var paymentMethoderror = {"items":[{name: 'Payment Method', message: Hypr.getLabel('paymentMethodValidation')}]};
                        if(isCreditCardPayment){
                           paymentMethoderror = {"items":[{name: 'Payment Method', message: Hypr.getLabel('paymentMethodCardValidation')}]};
                        }
                        
                        order.onCheckoutError(paymentMethoderror);
                    }
                    else {
                        
                        if(_.isUndefined(window.order.get("splitPayments")) || _.isEmpty(window.order.get("splitPayments"))) {
                            if(_.isUndefined(window.order.get("isBlueAmountOrderTotal")) || !window.order.get("isBlueAmountOrderTotal")) {
                                this.checkAmountRequested(order);
                            }
                        }
                        
                        if(!_.isUndefined(billingInfo.get("card")) && !isCreditCardPayment && !isPaidByBluePrepaidCard && !window.order.get("isCOD") && !window.order.get('isWalletDeducation')) {
                            var DefaultpaymentMethoderror = {"items":[{name: 'Payment Method', message: Hypr.getLabel('paymentMethodValidation')}]};
                            order.onCheckoutError(DefaultpaymentMethoderror);
                        }
                    }
                    if(isCreditCardPayment && !isPaidByBluePrepaidCard && !window.order.get("isCOD")) {
                        if(billingInfo.get('card') && billingInfo.get('card').get('cvv') === undefined){
                            var CheckoutError = {"items":[{name: 'card.cvv', message: Hypr.getLabel('cvvValidation')}]};
                            $('.saved-cvv-validation-msg').text(Hypr.getLabel('cvvValidation'));
                            order.onCheckoutError(CheckoutError);
                        }
                        if(billingInfo.get('card') && billingInfo.get('card').get('cvv') ==='' && billingInfo.get('card').get('cardNumber')=== undefined){
                            var Carderror = {"items":[{name: 'card.cvv', message: Hypr.getLabel('cvvValidation')}]};
                            $('.saved-cvv-validation-msg').text(Hypr.getLabel('cvvValidation'));
                            order.onCheckoutError(Carderror);
                        }
                        if(billingInfo.get('card') && billingInfo.get('card').get('cvv') ===''){
                            var error = {"items":[{name: 'card.cvv', message: Hypr.getLabel('cvvValidation')}]};
                            $('.saved-cvv-validation-msg').text(Hypr.getLabel('cvvValidation'));
                ...
                if(isCreditCardPayment && !isPaidByBluePrepaidCard) {
                    if ( ((nonStoreCreditOrGiftCardTotal > 0 && this.validate() && !hasTokenPayment) || this.validateReviewCheckoutFields()) && ( !this.isNonMozuCheckout())) {
                        this.isSubmitting = false;
                        return false;
                    }
                }
                this.isLoading(true);
                process.push(this.disbalePlaceOrderBtn);
               // $('.place-order-btn').addClass('loader-btn');
               // $('.place-order-btn').addClass('hidden');
                ...
                if(isCreditCardPayment) {
                    // var saveCreditCard = true; // Changining This As thi will be auto true/false once we will get credit
                    //var saveCreditCard = false;
                    var saveCreditCard = false;
                  //  console.log("is this coming finally here" + creditCard.billingInfo.card.paymentOrCardType.toLowerCase());
                    var selectASavedCardCookie = $.cookie('checkboxState');
                    if(selectASavedCardCookie == 'checked'){
                        saveCreditCard = true;
                    }
                    if (activePayments !== null && activePayments.length > 0) {
                        ...
                        creditCardFromSplitPayment = this.checkAndGetCreditCardFromSplitPayment();
                        
                        if(_.isUndefined(creditCard) && creditCardFromSplitPayment) {
                            creditCard = {};
                            creditCard.billingInfo = creditCardFromSplitPayment;
                        }

                        if (creditCard && creditCard.billingInfo && creditCard.billingInfo.card) {
                            
                            if(creditCard.billingInfo.card.paymentOrCardType.toLowerCase() == 'knetcard') saveCreditCard = false; //If payment Type Is KNET Then We Need Not To Save Card
                            if(creditCard.billingInfo.card.paymentOrCardType.toLowerCase() == 'applepay') saveCreditCard = false; //If payment Type Is KNET Then We Need Not To Save Car
                             if(creditCard.billingInfo.card.paymentOrCardType.toLowerCase() == 'benefitcard') saveCreditCard = false;
                             if(creditCard.billingInfo.card.paymentOrCardType.toLowerCase() == 'qpay') saveCreditCard = false;
                             if(creditCard.billingInfo.card.paymentOrCardType.toLowerCase() == 'omannet') saveCreditCard = false;
                ...
                if (!this.isNonMozuCheckout() && isAuthenticated || isSavingNewCustomer && this.hasRequiredBehavior(1014)) {
                    if(billingInfo.get("paymentType").toLowerCase() === "check") {
                 ...
                 api.steps(process).then(this.onCheckoutHandle, this.onCheckoutError);
                 
            //   }
            },
            checkAndGetCreditCardFromSplitPayment: function() {
                var  splitPaymentsArr = JSON.parse(sessionStorage.getItem('splitPayments'));
                if(!_.isEmpty(splitPaymentsArr)) return _.findWhere(splitPaymentsArr[1], { paymentType: 'CreditCard' });
            },
            disbalePlaceOrderBtn:function(){
                $('.place-order-btn').addClass('loader-btn');
                $('.place-order-btn span').hide();
            },
            update: function() {
            ...
            checkAmountRequested : function(orderModel) {
                
                var paidWithBlueWallet = orderModel.get('blueWallet'),
                    paidWithBluePrepaidCard = orderModel.get('bluePrepaidCard'),
                    orderTotal = orderModel.get("total"),
                    isError = false;

                if(paidWithBluePrepaidCard && paidWithBlueWallet) {
                    isError = (paidWithBlueWallet + paidWithBluePrepaidCard) < orderTotal ? true : false;
                } 
                else  {
                    if(paidWithBluePrepaidCard) isError = paidWithBluePrepaidCard < orderTotal ? true : false;

                    if(paidWithBlueWallet) isError = paidWithBlueWallet < orderTotal ? true : false;
                }

                if(isError) {
                    var invalidRequestedAmount = {"items":[{name: 'Invalid Request Amount', message: 'Requested Amount Is Less Than Order Total'}]};
                    orderModel.onCheckoutError(invalidRequestedAmount);
                }
